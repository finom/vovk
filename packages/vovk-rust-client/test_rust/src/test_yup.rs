...
pub mod test_yup {
    ...
    fn test_complaints() {
        ...
        let no_complaints: with_yup_client_controller_rpc::handle_schema_complaints_::body = 
            serde_json::from_value(get_complaining_object(None)).unwrap();
        
        // Test valid object first
        let result = with_yup_client_controller_rpc::handle_schema_complaints(
            no_complaints.clone(),
            (),
            (),
            None,
        ...
        if let serde_json::Value::Object(map) = no_complaints_json {
            ...
            let complaining_object: with_yup_client_controller_rpc::handle_schema_complaints_::body = 
                serde_json::from_value(get_complaining_object(Some(key.clone()))).unwrap();
            
            // Test with client validation disabled (server-side error)
            let result_server = with_yup_client_controller_rpc::handle_schema_complaints(
            ...
            let result_client = with_yup_client_controller_rpc::handle_schema_complaints(
                complaining_object,
                (),
                (),
                None,
    ...
    fn test_type_complaints() {
        ...
        if let serde_json::Value::Object(ref map) = no_complaints_json {
            ...
            for (key, _) in map.clone() {
                ...
                let result = serde_json::from_value::<with_yup_client_controller_rpc::handle_schema_complaints_::body>(complaining_object);
