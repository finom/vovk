...
await describe('Custom client templates', async () => {
  ...
  await it('Basic check', async () => {
    ...
    await runAtProjectDir(`../dist/index.mjs generate --full-schema --template ${templatesDir}/hello-world.js --out ${compiledClientFolderName}`);

    await assertFile(`${compiledClientFolderName}/hello-world.js`, [
      `Hello, World!`,
    ]);
  });
  ...
  await it('Should generate client with custom templates', async () => {
    ...
    await assertFile('client-from-template/foo.js', [
      `Hello, World!`,
    ]);
  });
