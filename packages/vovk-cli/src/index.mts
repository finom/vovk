...
program
  ...
  .option('--out, --full-out <path>', 'path to output directory for full client')
  .option('--from, --full-from <templates...>', 'client template names for full client')
  .option('--full-only', 'generate only full client even if segmented client is enabled')
  .option('--segmented-only', 'generate only segmented client even if full client is enabled')
