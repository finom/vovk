...
- `package.json` `dependencies` gained `vovk` + `vovk-ajv` (always) + chosen server-side validation library. For `zod`: `zod`. For `valibot`: `valibot` + `@valibot/to-json-schema`. For `arktype`: `arktype`. `vovk-ajv` is the client-side validator regardless of server-side choice.
...
- `.gitignore` gained the generated client dir (`src/client/`, or `client/` for root-layout projects). Client is regenerated by `prebuild`, no need to commit it; import via `@/client`.
...
### 5. Verify the `@/client` alias resolves

Composed client generates into `src/client/` (or `client/` for root-layout projects); canonical import is `@/client`. Standard Next.js scaffold already has `compilerOptions.paths` with `"@/*": ["./src/*"]` (or `["./*"]` for root layout), so nothing to do. If the project has no `@/*` alias or it points elsewhere, add an explicit entry:
...
Specific `@/client` entry takes precedence over wildcard, so safe alongside any existing `@/*`. Root-layout variant: `"@/client": ["./client"]`. Projects that refuse aliases can import via relative path instead.

### 6. Offer the follow-up steps

Summarize concisely: what got installed (incl. validation library from step 2), what changed, what scripts exist now, and that composed client generates into `src/client/` (gitignored, rebuilt by `prebuild`) with imports from `@/client`. Then ask about two natural next steps — don't cascade silently.
